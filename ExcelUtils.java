...
public class ExcelUtils {
	...
	// 表头的英文字段需对应VO里的变量名,例：表头Turple<String, String> t = new Turple<String,
	// String>("matchName", "赛事名称"); matchName对应matchVO里的变量 matchName
	...
	public static void excelExport(List dataList, List<Turple<String, String>> headerList, String name,
			HttpServletResponse response) throws NoSuchFieldException, SecurityException, IllegalArgumentException,
	...
	private static int getSheetNum(List<Object> voList) {
		BigDecimal num = new BigDecimal(voList.size()).divide(new BigDecimal(SHEET_MAX_CNT + ""),
				BigDecimal.ROUND_CEILING);
		return Integer.valueOf(num.toString());
	}

	@SuppressWarnings("rawtypes")
	public static HSSFWorkbook createExcel(List<Object> dataList, List<Turple<String, String>> headerList)
		...
		for (int s = 1; s <= sheetNum; s++) {
			...
			for (int i = a, ro = 0; i < dataList.size(); i++, ro++)// i层循环决定有多少行
			{
				...
				// if(new BigDecimal(i+"").equals(new
				// BigDecimal(SHEET_MAX_CNT+"").multiply(new
				// BigDecimal(s+"").subtract(new BigDecimal("1")))))
				{
					a = i + 1;
	...
	// 获取表头的英文和汉字
	public static List<Object> getHeader(List<Turple<String, String>> headerList, HSSFSheet sheet)
			throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
